  ...
  return (
    <button
      ...
      className={`p-1.5 rounded hover:bg-gray-200 transition-colors ${
        active ? "bg-gray-200 text-slate-800" : "text-gray-600"
      }`}
    >
      {children}
    </button>
  ...
  return (
    <div className="relative">
      <button
        ...
        title={title}
        className={`p-1.5 rounded hover:bg-gray-200 transition-colors ${
          open ? "bg-gray-200 text-slate-800" : "text-gray-600"
        }`}
      >
        {icon}
      </button>
      {open && (
        ...
        >
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-1.5 text-xs text-blue-600 hover:bg-blue-50 border border-blue-200 rounded-md py-1.5 mb-2 transition-colors"
          >
            <SwatchBook size={13} />
  ...
  return (
    <div className="border border-gray-300 rounded-lg bg-white">
      <div className="sticky top-0 z-30 border-b border-gray-200 bg-gray-50 px-2 py-1.5 flex flex-wrap items-center gap-0.5 rounded-t-lg">
        ...
        <ToolbarButton onClick={() => editor.chain().focus().setTextAlign("justify").run()} active={editor.isActive({ textAlign: "justify" })} title="Justify"><AlignJustify size={16} /></ToolbarButton>

        <span className="w-px h-5 bg-gray-300 mx-1" />

        <ToolbarColorPopover icon={<Palette size={16} />} title="Text Color" onApply={(hex) => editor.chain().focus().setColor(hex).run()} onUnset={() => editor.chain().focus().unsetColor().run()} />
        <ToolbarColorPopover icon={<Highlighter size={16} />} title="Background Color" onApply={(hex) => editor.chain().focus().setBackgroundColor(hex).run()} onUnset={() => editor.chain().focus().unsetBackgroundColor().run()} />
        ...
        <ToolbarButton onClick={insertTable} title="Table"><TableIcon size={16} /></ToolbarButton>
