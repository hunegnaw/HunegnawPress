...
#################################################################
# HunegnawPress — Unified Deployment Script
#
# Deploys to Cloudways PHP stack with Node.js via PM2 + .htaccess proxy
# One codebase, multiple sites — each site has its own config directory.
#
# Usage: ./scripts/deploy.sh <site-dir> [staging|production]
#
# Examples:
#   ./scripts/deploy.sh ~/Clients/cherryoakracing.com production
#   ./scripts/deploy.sh ~/Clients/thesnotty.com production
#   ./scripts/deploy.sh ~/Clients/cherryoakracing.com staging
#
# The site directory must contain .env.production or .env.staging with:
#   - SERVER_USER, SERVER_HOST, SERVER_PATH, SSH_KEY
#   - DB_USER, DB_PASS, DB_NAME
#   - APP_NAME (PM2 process name, e.g. "cherryoakracing")
#   - APP_PORT (e.g. 4000)
...
load_environment() {
    # Resolve site directory to absolute path
    SITE_DIR=$(cd "$SITE_DIR" 2>/dev/null && pwd || echo "$SITE_DIR")

    local env_file="$SITE_DIR/.env.$ENVIRONMENT"
    if [ ! -f "$env_file" ]; then
    ...
    local required_vars=("SERVER_USER" "SERVER_HOST" "SERVER_PATH" "SSH_KEY" "DB_USER" "DB_PASS" "DB_NAME" "APP_NAME" "APP_PORT")
    for var in "${required_vars[@]}"; do
        if [ -z "${!var}" ]; then error "Missing: $var in $env_file"; exit 1; fi
    done
    ...
    # Derive site name from directory for display
...
show_header() {
    ...
    if [ "$ENVIRONMENT" = "production" ]; then
        echo -e "${GREEN}╔══════════════════════════════════════════════════════════════╗${NC}"
        echo -e "${GREEN}║     HunegnawPress — PRODUCTION Deployment                   ║${NC}"
        echo -e "${GREEN}║     Site: $(printf '%-49s' "$SITE_NAME")║${NC}"
        echo -e "${GREEN}╚══════════════════════════════════════════════════════════════╝${NC}"
    else
        echo -e "${YELLOW}╔══════════════════════════════════════════════════════════════╗${NC}"
        echo -e "${YELLOW}║     HunegnawPress — Staging Deployment                      ║${NC}"
        echo -e "${YELLOW}║     Site: $(printf '%-49s' "$SITE_NAME")║${NC}"
        echo -e "${YELLOW}╚══════════════════════════════════════════════════════════════╝${NC}"
    fi
    echo ""
}
...
confirm_deploy() {
    ...
    echo "  DB:      $DB_NAME"
    echo "  PM2:     $APP_NAME (port $APP_PORT)"
    echo ""
...
ENDSSH
    ...
    LOCAL_BACKUP_DIR="$SITE_DIR/backups/$ENVIRONMENT/$DEPLOY_DATE"
    ...
    scp -i "$SSH_KEY" "$SERVER_USER@$SERVER_HOST:/home/master/backups/$DEPLOY_DATE/database.sql" "$LOCAL_BACKUP_DIR/database.sql" 2>/dev/null && \
        success "Local backup saved to $LOCAL_BACKUP_DIR/" || \
...
deploy_files() {
    ...
    local env_file="$SITE_DIR/.env.$ENVIRONMENT"
    grep -v -E '^(SERVER_USER|SERVER_HOST|SERVER_PATH|SSH_KEY|BRANCH|DB_USER|DB_PASS|DB_NAME|DB_HOST|APP_NAME|APP_PORT|STAGING_PORT|NOTIFY_EMAIL|SLACK_WEBHOOK|HEALTH_CHECK_HOST)=' "$env_file" | \
    grep -v '^#' | grep -v '^$' | \
...
activate_release() {
    ...
    ssh -T -i "$SSH_KEY" "$SERVER_USER@$SERVER_HOST" bash -s "$CURRENT_LINK" "$DEPLOY_DIR" "$RELEASES_DIR" "$SHARED_DIR" "$ENVIRONMENT" "$DB_HOST" "$DB_USER" "$DB_PASS" "$DB_NAME" "$APP_NAME" "$APP_PORT" << 'ENDSSH'
set -e
CURRENT_LINK="$1"
DEPLOY_DIR="$2"
RELEASES_DIR="$3"
...
PM2_APP_NAME="$APP_NAME-$ENVIRONMENT"
echo "PM2 app: $PM2_APP_NAME (port $APP_PORT)"
...
if [ -n "$PM2_CMD" ]; then
    ...
    # Set the port in the environment for this app
    export PORT="$APP_PORT"

    $PM2_CMD delete "$PM2_APP_NAME" 2>/dev/null || true
    if [ -f "ecosystem.config.js" ]; then
        # Override the app name and port via environment
        APP_NAME="$APP_NAME" APP_PORT="$APP_PORT" ENVIRONMENT="$ENVIRONMENT" \
            $PM2_CMD start ecosystem.config.js --only "$PM2_APP_NAME"
    else
        PORT="$APP_PORT" $PM2_CMD start npm --name "$PM2_APP_NAME" -- start
    fi
    $PM2_CMD save
    echo "PM2 started"
    $PM2_CMD list
...
show_summary() {
    ...
    echo ""
    echo "  Rollback: ./scripts/rollback.sh $SITE_DIR $ENVIRONMENT"
    echo ""
...
main() {
    if [ -z "$SITE_DIR" ]; then
        echo "Usage: ./scripts/deploy.sh <site-dir> [staging|production]"
        echo ""
        echo "Examples:"
        echo "  ./scripts/deploy.sh ~/Clients/cherryoakracing.com production"
        echo "  ./scripts/deploy.sh ~/Clients/thesnotty.com production"
        exit 1
    fi
    ...
    if [ "$ENVIRONMENT" != "staging" ] && [ "$ENVIRONMENT" != "production" ]; then
        echo "Usage: ./scripts/deploy.sh <site-dir> [staging|production]"; exit 1
    fi
